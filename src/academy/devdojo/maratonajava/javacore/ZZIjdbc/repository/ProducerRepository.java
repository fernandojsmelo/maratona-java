...
//        DELETE FROM `anime_store`.`producer` WHERE (`id` = '16');
    ...
    public static void saveTransaction(List<Producer> producers) {
        ...
        try (Connection conn = ConnectFactory.getConnection()) {
            ...
            log.error("Error while trying to save produce '{}'", producers, e);
        }

    }

    private static void preparedStatementSaveTransaction(
            Connection conn, List<Producer> producers) throws SQLException {
        String sql = "INSERT INTO `anime_store`.`producer` (`name`) VALUES (?);";
        boolean shouldRollback = false;
        for (Producer p : producers) {
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
                log.error("Saving producer '{}'", p.getName());
