...
public class StoreServiceWithDiscount {
    ...
    public String getPriceSync(String storeNme) {
        double price = princeGeneretor();
        Discount.Code discountCode = Discount.Code.values()[
                ThreadLocalRandom.current().nextInt(Discount.Code.values().length)
                ];
        return String.format("%s:%.2f:%s", storeNme, price, discountCode);
    }

    public String applyDiscount(Quote quote) {
        delay();
        double discountValue = quote.getPrice() *
                (100 - quote.getDiscountCode().getPercentage()) / 100;
        return String.format(
                "%s original price: %.2f. Applying discount code %s. Final price: %.2f",
                quote.getStore(),
                quote.getPrice(),
